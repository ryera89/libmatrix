...
inline std::enable_if_t<(mtype == MATRIX_TYPE::CSR || mtype == MATRIX_TYPE::SCSR ||
                         mtype == MATRIX_TYPE::HCSR),Matrix<T,2,mtype>>
sparse_add(const Matrix<T,2,mtype> &sm1,const Matrix<T,2,mtype> &sm2){
    ...
    if constexpr (mtype == MATRIX_TYPE::CSR){
        ...
        return Matrix<T,2,mtype>(rows,move(rows_start),move(rows_end),move(columns_index),move(vals));
    }
...
inline std::enable_if_t<(mtype == MATRIX_TYPE::CSR || mtype == MATRIX_TYPE::SCSR ||
                         mtype == MATRIX_TYPE::HCSR),Matrix<T,2,mtype>>
sparse_sub(const Matrix<T,2,mtype> &sm1,const Matrix<T,2,mtype> &sm2){
    ...
    return Matrix<T,2,mtype>(rows,cols,move(rows_start),move(rows_end),move(columns_index),move(vals));
}
template<typename T,MATRIX_TYPE mtype>
inline std::enable_if_t<(mtype == MATRIX_TYPE::CSR || mtype == MATRIX_TYPE::SCSR ||
                         mtype == MATRIX_TYPE::HCSR),Matrix<T,1>>
sparse_matrix_vector_prod(const Matrix<double,2,mtype> &sm,const Matrix<T,1> &v){
    ...
    if constexpr (is_same_v<T,float>){
        ...
        status = mkl_sparse_s_dotmv(SPARSE_OPERATION_NON_TRANSPOSE,alpha,sm.sparse_matrix_handler()
                                                                               ,sm.descr(),v.begin(),beta,res.begin(),&d);
        check_sparse_operation_status(status);
    }else if constexpr (is_same_v<T,double>) {
        double alpha = 1;
        double beta = 0;
        status = mkl_sparse_d_dotmv(SPARSE_OPERATION_NON_TRANSPOSE,
                                    alpha,sm.sparse_matrix_handler(),sm.descr(),v.begin(),beta,res.begin(),&d);
        check_sparse_operation_status(status);
...
inline std::enable_if_t<(mtype == MATRIX_TYPE::CSR || mtype == MATRIX_TYPE::SCSR ||
                         mtype == MATRIX_TYPE::HCSR),Matrix<T,1>>
vector_sparse_matrix_prod(const Matrix<T,1> &v,const Matrix<double,2,mtype> &sm){
    assert(sm.cols() == v.size());
    //matrix_descr descr;
    //descr.type = SPARSE_MATRIX_TYPE_GENERAL;
    Matrix<T,1> res(sm.rows());
...
inline enable_if_t<is_number<Scalar>(), conditional_t<is_complex<Scalar>() && (mtype == MATRIX_TYPE::HER),
                                                      Matrix<RT,N,MATRIX_TYPE::GEN>,Matrix<RT,N,mtype>>>
operator +(const Scalar &val,const Matrix<T,N,mtype> &m){
...
inline enable_if_t<is_number<Scalar>(), conditional_t<is_complex<Scalar>() && (mtype == MATRIX_TYPE::HER),
                                                      Matrix<RT,N,MATRIX_TYPE::GEN>,Matrix<RT,N,mtype>>>
operator +(const Scalar &val,Matrix<T,N,mtype> &&m){
...
inline enable_if_t<is_number<Scalar>(), conditional_t<is_complex<Scalar>() && (mtype == MATRIX_TYPE::HER),
                                                      Matrix<RT,N,MATRIX_TYPE::GEN>,Matrix<RT,N,mtype>>>
operator +(const Matrix<T,N,mtype> &m,const Scalar &val){
...
inline enable_if_t<is_number<Scalar>(), conditional_t<is_complex<Scalar>() && (mtype == MATRIX_TYPE::HER),
                                                      Matrix<RT,N,MATRIX_TYPE::GEN>,Matrix<RT,N,mtype>>>
operator +(Matrix<T,N,mtype> &&m,const Scalar &val){
...
inline auto operator +(const Matrix<T,N,mtype1> &m1,const Matrix<U,N,mtype2> &m2){
    if constexpr (mtype1 == mtype2){
        if constexpr (mtype1 == MATRIX_TYPE::CSR || mtype1 == MATRIX_TYPE::SCSR || mtype1 == MATRIX_TYPE::HCSR){
...
inline Matrix<T,1> operator *(const Matrix<T,2,mtype> &m,const Matrix<T,1> &v){
    ...
    if constexpr (mtype == MATRIX_TYPE::CSR || mtype == MATRIX_TYPE::SCSR || mtype == MATRIX_TYPE::HCSR){
...
inline Matrix<T,1> operator *(const Matrix<T,1> &v, const Matrix<T,2,mtype> &m){
    ...
    if constexpr (mtype == MATRIX_TYPE::HER){
        ...
        double beta = 0.0;
        if constexpr (is_same_v<T,complex<float>>){
            cblas_chpmv(CblasColMajor,CblasLower,m.rows(),&alpha,m.begin(),v.begin(),1,&beta,r.begin(),1);
            return r;
        }
        if constexpr (is_same_v<T,complex<double>>){
            cblas_zhpmv(CblasColMajor,CblasLower,m.rows(),&alpha,m.begin(),v.begin(),1,&beta,r.begin(),1);
            return r;
        }
...
inline Matrix<T,2> operator *(const Matrix<T,2,MATRIX_TYPE::CSR> &sm,const Matrix<T,2> &dm){
    ...
    if constexpr (is_same_v<T,float>){
        status = mkl_sparse_s_mm(op,1,sm.sparse_matrix_handler(),sm.descr(),sparseLayout,dm.begin(),col,col,0,R.begin(),col);
    }else if constexpr (is_same_v<T,double>) {
        status = mkl_sparse_d_mm(op,1,sm.sparse_matrix_handler(),sm.descr(),sparseLayout,dm.begin(),col,col,0,R.begin(),col);

    }else if constexpr (is_same_v<T,complex<float>>) {
        complex<float> alpha(1,0);
        complex<float> beta(0,0);
        status = mkl_sparse_c_mm(op,alpha,sm.sparse_matrix_handler(),sm.descr(),sparseLayout,dm.begin(),col,col,beta,R.begin(),col);
    }else if constexpr (is_same_v<T,complex<double>>) {
        complex<double> alpha(1,0);
        complex<double> beta(0,0);
        status = mkl_sparse_z_mm(op,alpha,sm.sparse_matrix_handler(),sm.descr(),sparseLayout,dm.begin(),col,col,beta,R.begin(),col);
    }
    check_sparse_operation_status(status);
    return R;
}
template<typename T>
inline Matrix<T,2> operator *(const Matrix<T,2,MATRIX_TYPE::SCSR> &sm,const Matrix<T,2> &dm){
    assert(sm.cols() == dm.rows());
...
template<typename T>
inline Matrix<T,2> operator *(const Matrix<T,2,MATRIX_TYPE::HCSR> &sm,const Matrix<T,2> &dm){
    assert(sm.cols() == dm.rows());
...
inline Matrix<T,2> operator *(const Matrix<T,2> &dm,const Matrix<T,2,MATRIX_TYPE::CSR> &sm){
    ...
    if constexpr (is_same_v<T,float>){
        status = mkl_sparse_s_mm(op,1,sm.sparse_matrix_handler(),sm.descr(),sparseLayout,dm.begin(),row,dm.cols(),0,R.begin(),dm.cols());
    }else if constexpr (is_same_v<T,double>) {
        status = mkl_sparse_d_mm(op,1,sm.sparse_matrix_handler(),sm.descr(),sparseLayout,dm.begin(),row,dm.cols(),0,R.begin(),dm.cols());

    }else if constexpr (is_same_v<T,complex<float>>) {
        complex<float> alpha(1,0);
        complex<float> beta(0,0);
        status = mkl_sparse_c_mm(op,alpha,sm.sparse_matrix_handler(),sm.descr(),sparseLayout,dm.begin(),row,dm.cols(),beta,R.begin(),dm.cols());
    }else if constexpr (is_same_v<T,complex<double>>) {
        complex<double> alpha(1,0);
        complex<double> beta(0,0);
        status = mkl_sparse_z_mm(op,alpha,sm.sparse_matrix_handler(),sm.descr(),sparseLayout,dm.begin(),row,dm.cols(),beta,R.begin(),dm.cols());
    }
    check_sparse_operation_status(status);
    return R;
}
template<typename T>
inline Matrix<T,2> operator *(const Matrix<T,2> &dm,const Matrix<T,2,MATRIX_TYPE::SCSR> &sm){
    assert(dm.cols() == sm.rows());
...
template<typename T>
inline Matrix<T,2> operator *(const Matrix<T,2> &dm,const Matrix<T,2,MATRIX_TYPE::HCSR> &sm){
    assert(dm.cols() == sm.rows());
