...
private:
    ...
    void initSparseMatrixHandler(){
        ...
        m_descr.type = SPARSE_MATRIX_TYPE_GENERAL;
    }
public:
    ...
    const matrix_descr descr() const{return m_descr;}
...
private:
    ...
    vector<int_t> m_rows_start; //most recent representation
    vector<int_t> m_rows_end; //most recent representation
    vector<int_t> m_rowsIndex; //3 array representation
    vector<int_t> m_columns; //index de las columnas
    vector<T> m_elems; //elementos
    sparse_matrix_t m_handler;
    matrix_descr m_descr;
    ...
    void initSparseMatrixHandler(){
        ...
        check_sparse_operation_status(status);
        m_descr.type = SPARSE_MATRIX_TYPE_SYMMETRIC;
        m_descr.mode = SPARSE_FILL_MODE_UPPER;
        m_descr.diag = SPARSE_DIAG_NON_UNIT;
    }
public:
    static constexpr size_t order = 2;
    static constexpr  sparse_index_base_t m_index = SPARSE_INDEX_BASE_ZERO;   //base index
    static constexpr MATRIX_TYPE matrix_type = MATRIX_TYPE::SCSR;
    using value_type = T;
    using iterator = typename vector<T>::iterator;
    using const_iterator = typename vector<T>::const_iterator;
    ...
    Matrix(uint32_t n,const vector<int_t> &rows_start,const vector<int_t> &rows_end,
           const vector<int_t> &columns,const vector<T> &vals):m_rows(n),m_cols(n),m_rows_start(rows_start),
        m_rows_end(rows_end),m_rowsIndex(rows_start),m_columns(columns),m_elems(vals){
    ...
    Matrix(uint32_t n,vector<int_t> &&rows_start,vector<int_t> &&rows_end,vector<int_t> &&columns,
           vector<T> &&vals):m_rows(n),m_cols(n),m_rows_start(rows_start),m_rows_end(rows_end),m_rowsIndex(m_rows_start),
        m_columns(columns), m_elems(vals){
        assert(m_columns.size() == m_elems.size() && m_rows == m_rows_start.size() && m_rows == m_rows_end.size());
        m_rowsIndex.push_back(m_elems.size());
        shrinkToFitDataVectors();
    ...
    Matrix(const Matrix<T,2,MATRIX_TYPE::SYMM> &m):m_rows(m.rows()),m_cols(m.cols()),m_rows_start(m_rows),
        m_rows_end(m_rows){
        for (size_t i = 0; i < m_rows; ++i){
            bool row_first_nonzero = true;
            for (size_t j = i; j < m_cols; ++j){
                T val = m(i,j);
                if (val != T() || i==j){ //si el valor es distinto de cero o esta en la diagonal
                    m_elems.push_back(val);
                    m_columns.push_back(j);
                    if (row_first_nonzero){ //se ejecuta maximo solo una ves del loop principal i
                        m_rows_start[i] = m_elems.size()-1;
    ...
    Matrix& operator=(const Matrix<T,2,MATRIX_TYPE::SYMM> &m){
        m_rows = m.rows();
        m_cols = m.cols();
        m_rows_start.resize(m_rows);
        m_rows_end.resize(m_rows);
        ...
        for (size_t i = 0; i < m_rows; ++i){
            ...
            for (size_t j = i; j < m_cols; ++j){
                T val = m(i,j);
                if (val != T() || i==j){ //si el valor es distinto de cero o elemento esta en la diagonal
                    m_elems.push_back(val);
                    m_columns.push_back(j);
                    if (row_first_nonzero){ //se ejecuta maximo solo una ves del loop principal i
                        m_rows_start[i] = m_elems.size()-1;
    ...
    //TODO: revisar el algoritmo aca... pensar ordenar el vector de indices de columna de entrada
    Matrix<T,2,MATRIX_TYPE::CSR> operator()(const vector<uint32_t> &iindex,const vector<uint32_t> &jindex) const{
        uint32_t nrow = iindex.size();
        uint32_t ncol = jindex.size();
        vector<T> elems;
        elems.reserve(m_elems.size()); //avoiding new memory allocation
    ...
    //TODO: eliminar esta funcion una ves se terminen las pruebas
    void printData() const{
        for (size_t i = 0; i < m_rows_start.size(); ++i){
            uint32_t beg = m_rows_start[i];
            uint32_t end = m_rows_end[i];
...
private:
    ...
    void initSparseMatrixHandler(){
        ...
        check_sparse_operation_status(status);
        m_descr.type = SPARSE_MATRIX_TYPE_HERMITIAN;
        m_descr.mode = SPARSE_FILL_MODE_UPPER;
        m_descr.diag = SPARSE_DIAG_NON_UNIT;
    }
public:
    static constexpr size_t order = 2;
    static constexpr  sparse_index_base_t m_index = SPARSE_INDEX_BASE_ZERO;   //base index
    static constexpr MATRIX_TYPE matrix_type = MATRIX_TYPE::HCSR;
    using value_type = T;
    using iterator = typename vector<T>::iterator;
    using const_iterator = typename vector<T>::const_iterator;
    ...
    Matrix(const Matrix<T,2,MATRIX_TYPE::HER> &m):m_rows(m.rows()),m_cols(m.cols()),m_rows_start(m_rows),
        m_rows_end(m_rows){
        for (size_t i = 0; i < m_rows; ++i){
            bool row_first_nonzero = true;
            for (size_t j = i; j < m_cols; ++j){
    ...
    Matrix& operator=(const Matrix<T,2,MATRIX_TYPE::HER> &m){
        m_rows = m.rows();
        m_cols = m.cols();
        m_rows_start.resize(m_rows);
        m_rows_end.resize(m_rows);
    ...
    enable_if_t<is_arithmetic_v<Scalar>,Matrix&> operator*=(const T &val){
        for_each(m_elems.begin(),m_elems.end(),[&val](T &ele){ele*=val;});
        return *this;
    }
    template<typename Scalar>
    ...
    const T& operator()(size_t i,size_t j) const{
        ...
        if (it != last && *it == j){
            ...
            if (ii <= jj){
               return m_elems[pos];
            }else{ //retorna el conjugado.
                m_aux = std::conj(m_elems[pos]);
                return m_aux;
            }
        }
        return zero_val;
